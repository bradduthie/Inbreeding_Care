...
% TODO: The suggested sentence was confusing to me. The opportunity cost isn't caused by inbreeding, it's caused by monogamy. If we're going to have a paragraph like this that is specifically focused on opportunity costs, I think the relationship between costs and monogamy is what needs to be emphasised.

% TODO: Again, this is confusing to me. It's as if we're listing things that we need to consider for a theory that we haven't yet developed. The things that we need to consider really should *follow* from the key assumptions of the model, not just be things that we happen to think might be important for one reason or another (which is how this now reads to me). Else it comes across as if we're really just developing verbal theory in the Introduction (and concluding that X or Y is important), then expressing it in mathematical terms in the Model. Instead, I think it's better to introduce the general concepts focal to the theory, then *use* mathematics as tool throughout the Model to develop theory further. I've tried to reword the suggestion to see how this paragraph looks, but I still don't think that this really fits.
...
Theory can inform empirical hypothesis testing by logically connecting assumptions to novel empirical predictions. We demonstrate that given a small number of assumptions regarding PI and inbreeding, selection will cause PI to increase with increasing relatedness between parents and increasing magnitude of ID (Fig. \ref{mcurves_uni}B). The total number of offspring that inbreeding parents produce will correspondingly decrease given the fundamental trade-off between investment per offspring and total offspring production. Empirical studies are now needed to test key assumptions and predictions. % TODO: I like the suggested break-up of sentences, but since we've moved to empirical systems in the Discussion, I think writing out 'PI and offspring production' fits better than 'm and n'. 
...
$\zeta_{\textrm{off}}$  & Identical-by-descent allele copies inherited per viable offspring & \\
...
$\gamma$                & Rate at which a focal female's identical-by-descent alleles are \\
                        & inherited by viable offspring & \\
