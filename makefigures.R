...
points(x=xline,y=rep(yof000,length(xline)),type="l",lwd=1.5);
points(x=xline,y=rep(yof025,length(xline)),type="l",lwd=1.5);
yline  <- seq(from=0,to=yof000,by=0.001);
points(x=rep(imopt.f025,length(yline)),y=yline,type="l",lwd=1.5);
...
# ============================================================================
# Now produce combined versions of the figures below for ProcB
# ============================================================================
# First Figure 1
#-----------------------------------------
setEPS();
...
arrows(x0=r00m,x1=r00m,y0=0.1,y1=0.003,length = 0.08,angle=30,code=2,lwd=1.25);
arrows(x0=r05m,x1=r05m,y0=0.1,y1=0.003,length = 0.08,angle=30,code=2,lwd=1.25);
text(x=r00m,y=0.145,labels=expression(paste(italic(m),"*"[r==0])),cex=1.20);
text(x=r05m,y=0.145,labels=expression(paste(italic(m),"*"[r==1/2])),cex=1.20);
#----------------------------------------------------------------
plot(x=bss,y=rg000$gam,type="l",lwd=2,ylim=c(0.09,0.22),pch=1.5,yaxt="n",
     xlab=expression(paste("Inbreeding depression (",beta,")")),
...
text(x=bss[18],y=rg250$gam[18]+0.0038,labels="r=1/4",srt=-9,cex=1);
text(x=bss[18],y=rg500$gam[18]+0.0038,labels="r=1/2",srt=-12,cex=1);
text(x=4.90,y=0.213,labels="C",cex=2.5);
# --------------------------------------------------------------------
plot(x=bss,y=rg000$mvl,type="l",lwd=2,ylim=c(2,6),pch=1.5,
     xlab=expression(paste("Inbreeding depression (",beta,")")),
     ylab=expression(paste("Optimal PI (",italic(m),"*)")),
