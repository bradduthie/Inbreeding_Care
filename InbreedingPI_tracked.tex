...
To incorporate this cost, we explicitly consider both the direct and indirect fitness consequences of inbreeding. We assume that if a focal female avoids inbreeding with her male relative, then that relative will outbreed instead {\color{blue}(\cite{Duthie2015a} provide a potential framework for relaxing this assumption)}, and that parents are outbred ($f=0$) and invest optimally for any given $\beta$. We define $m^{*}_{0}$ and $m^{*}_{r}$ as optimal investment for outbreeding and for inbreeding to the degree $r$, respectively. Therefore, if a focal female avoids inbreeding,
...
{\color{blue}Inbreeding is predicted to increase relatedness among offspring, potentially affecting conflict among siblings over PI. Conflict among siblings might occur within or among broods \cite{Parker2002, Bonisoli-Alquati2011, Ruch2014}. Within broods, conflict over PI is directly proportional to relatedness among offspring following Hamilton's rule \cite{Hamilton1964, Hamilton1964a}, assuming total PI budget ($M$) is fixed per brood. A focal offspring thereby increases its inclusive fitness by obtaining additional PI if doing so has a marginal benefit that is greater than the average marginal benefit to its siblings, multiplied by the focal offspring's average relatedness to its siblings \cite{Parker2002}. Hence, intra-brood sibling conflict over PI will be lower for full-siblings than half-siblings, and conflict will further decrease if offspring are inbred. In contrast, conflict among broods between full versus half siblings depends on the mating system. If only females provide PI, offspring that obtain additional PI at the expense of their siblings only affect the inheritance of their siblings' maternally derived identical-by-descent alleles, meaning that conflict is not directly affected by whether or not siblings share a sire, but by the degree to which sires provide PI \cite{Lessells1999, Parker2002}. If offspring are outbred, the proportion of maternally-derived identical-by-descent alleles shared among siblings is always $1/4$, but this proportion will be higher given inbreeding, reducing conflict among inbred siblings. }




% (1) General sentence about sib-sib conflict, perhaps add that \cite{Gardner2011a} showed that sib competition increases with relatedness between sibling and therefore parental inbreeding. However, increased parental investment in the form of care might also increase sib-sib competition. Inbreeding and relatedness between siblings might also affect alloparental care \cite{Davies2016}. (2) Intra-brood conflict should be affected by relatedness between sibs. (3) Inter-brood conflict,  \cite{Lessells1999} and \cite{Parker2002} emphasise that mean relatedness between sibs should not affect conflict, but this has only been considered in the context of full versus half sib mating; it holds true because sib's mean relatedness to each parent remains 0.25. However, given inbreeding, this might change, so more work is needed to determine how interbrood sib-sib conflict is modulated by inbreeding.

% Here we have shown that.

% =============================================================

% \cite{Gardner2011a} Show that sib-sib competition exists as long as siblings are not clonal, but competition should decrease with increasing relatedness between siblings, and therefore increasing inbreeding of a parent. Increased food provisioning and nest safety promotes competition among siblings. Likewise, increased sibling competition promotes increased food provisioning and nest safety by parents.

% \cite{Davies2016} Show that inbreeding might affect evolution of sociality.

% \cite{Lessells1999} Show that production of full versus half siblings does not affect affect parent-offspring conflict because -- while offspring are related to half-siblings by 0.25 and full siblings by 0.5, you really need to think of it as an offspring being related to siblings by 0.25 through one (half sib) or both (full sib) parents. If half-sibs are produced, you need to ask what happened to the other parent's reproductive success. This, however, should not affect inbreeding's effect on sib conflict -- inbred siblings are simply more closely related through both parents, so conflict should decrease between siblings with inbreeding. 

% Get suggestions for measuring PI in the paragraph starting with ''One key assumption''. 

% In the same paragraph of ''One key assumption'', can distinguish more clearly between investment and care.

% Add a brief new paragraph showing how sibling conflict could be affected by PI and inbreeding, drawing from Gardner2011a noting that increasing PI could both increase conflict over resources provided, but also decrease conflict due to closer relatedness among siblings.

% Add a sentence mentioning explosion of sexual conflict and kin selection in recent years. ("Recently... much interest in .. ... ...").
