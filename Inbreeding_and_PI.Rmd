...
Further, the degree to which a female is inbred should have only slightly decrease $\gamma^{*}$. We can again consider outbreeding ($k=0$) and inbreeding between first order relatives ($k=1/4$), but assume $f=1/4$ (the female is the offspring of first order relatives) instead of $f=0$. If $k=0$ and $f=1/4$, then $\gamma^{*}=$ `r round(otang.f025,digits=3)`, versus $\gamma^{*}=$ `r round(otang.f000,digits=3)` given $f=0$. If $k=1/4$ and $f=1/4$, then $\gamma^{*}=$ `r round(itang.f025,digits=3)`, versus $\gamma^{*}=$ `r round(itang.f000,digits=3)` given $f=0$. We plot these lines in the figure below for $f=1/4$, which can be directly compared with the original curves in which $f=0$ is implicit.

```{r, echo=FALSE}
Alleles_IBD_k000 <- OffATf(k=0.0, f=0.25, m=PI, B0=1, B1=1, c=1);
par(mar=c(5,5,2,2));
plot(PI,Alleles_IBD_k000,type="l",lwd=3,ylim=c(0,1),
     xlab=expression(paste("Parental investment (",italic(m),")")),
     ylab=expression(paste("IBD alleles in offspring (",zeta[off],")")),
     cex.lab=1.5,cex.axis=1.5);
abline(h=0,lty="dotted",lwd=0.8);
Alleles_IBD_k025 <- OffATf(k=0.25, f=0.25, m=PI, B0=1, B1=1, c=1);
points(PI,Alleles_IBD_k025,type="l",lwd=3,col="black",lty="dashed");
abline(b=otang.f025,a=0,lty="solid",lwd=2);
abline(b=itang.f025,a=0,lty="dashed",lwd=2);
```

It is worth noting that because the optimal amount of parent investment always increases as inbreeding depression becomes more severe (Appendix 2), if increases in $f$ affect inbreeding depression of offspring, then $m^{*}$ is expected to increase. 
...
Here we show that optimal parental investment always increase with increasing inbreeding given inbreeding depression and $c>0$. Because we are only interested in the sign of this increase, it will be sufficient to show that $\partial r / \partial m^{*} > 0$ given $\beta>0$ and $c>0$. 

First, we note that $m^{*}$ is defined as the value of $m$ that maximises the rate of increase in $\zeta_{off}$ for a female. This is described by the line that passes through the origin and lies tangent to $\zeta_{off}(m)$. As in Appendix 1, we have the general equation for which $m=m^{*}$,
...
It is trivial to show that a female that inbreeds with a first order relative ($r=1/2$) has a higher fitness than a female that outbreeds ($r=0$) given $m_{min}=1$, $\beta=1$, and $c=1$ at optimal values of $m^{*}_{r=1/2}$ and $m^{*}_{r=0}$. To do this, we define $\delta$ as follows,
...
$$ \frac{n}{2}\left(1+r\right)\left(1-\delta_{r}\right). $$

In the above, $n$ is the total number of offspring a female produces in a reproductive bout. Biparental inbreeding models assume that this value is constant, but $n$ will scale linearly with $m$ (by definition) because females that invest more in each offspring (high $m$) will produce fewer total offspring (low $m$). To account for this, we can simply include $m$ to scale for offspring produced,

$$ \frac{n}{2 m}\left(1+r\right)\left(1-\delta_{r}\right). $$

Fitness given $r=0$ and $r=1/2$, and $m^{*}_{r=0}$ and $m^{*}_{r=1/2}$ can be determined by substituting some constant value for $n$ (for simplicity, assume $n=1$), as the magnitude of $n$ will not affect relative fitness differences. 

To show that inbreeding with first order relatives returns a higher fitness than outbreeding given the above conditions assumed in our model, we can simply use the above equation directly to compare the fitness given both $r=1/2$ and $r=0$, noting that $\delta_{r=1/2}=0.26$ and $\delta_{r=0}=0.32$ (note that $\delta_{r=1/2}<\delta_{r=0}$ because inbreeding parents are investing more in their offspring, $m^{*}_{r=1/2}=$ `r round(r05m,digits=2)` versus $m^{*}_{r=0}=$ `r round(r00m,digits=2)`). Consequently, we can use the above equation to show that the fitness gain of an optimally investing female that inbreeds with a first order relative is `r round((1/(r05m*2))*(1+0.5)*(1-exp(-1*(r05m - 1 -1*0.5))),digits=2)`, compared with `r round((1/(r00m*2))*(1+0.0)*(1-exp(-1*(r00m - 1 -1*0.0))),digits=2)` for the outbreeding female; these values are identical to our earlier calculated values of $\gamma^{*}_{r=1/2}$ and $\gamma^{*}_{r=1/2}$.
