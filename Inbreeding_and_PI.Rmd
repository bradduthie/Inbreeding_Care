...
<!--- It hadn't occurred to me that some readers might be confused about the point of doing inclusive theory by looking at individual's inclusive fitness given particular situations. I don't think the point is really to predict on what individuals might or might not do and the fitness consequences that follow, but instead to focus on how selection is predicted to act -- I've written it in this way now so that the connection between the fitness consequences for focal individuals are more clearly connected to the general goal of understanding natural selection, rather than on understanding what happens for focal individuals themselves. Previously, this last sentence read, ''Individuals that inbreed will increase their fitness by increasing their per offspring parental investment and decreasing their total reproductive output, relative to outbreeders.'' --->
...
<!--- Note in the paragraph below: One way to think about this type of theory paper is as a kind of methods paper, except that instead of a methods of analysis or laboratory protocol for addressing particular questions, this is a 'methods' for a way of thinking about inbreeding and PI. Readers need to see how the ideas in this manuscript build from earlier papers (ideally, develop an intuitive understanding through the mathematics), and not just be provided with the big predictions themselves. I think we want readers to use their newfound understanding of how to think about selection on inbreeding and PI should work to acheive a more general understanding of selection and how it might work across or within different biological systems, then be able to apply this new understanding directly instead of relying on us to predict the specifics. In other words, I think that the journey through the mathematics and key concepts is more important than the destination of the predictions. Hence, I think that discussing the logic of our model directly in the context of previous models is important. --->
...
<!--- I think that the above paragraph is important because it essentially shows the whole logic behind the modelling. It convinces the reader that we're not just pulling the above equation out of thin air, but that it is, in fact, derived from the same set of assumptions of both PI and inbreeding theory as we claimed. --->
...
A general relationship between $\beta$, $m^{*}$, and $\gamma^{*}$ for different values of $r$ can be determined numerically. In Fig. 1B, this relationship is shown across a range of $\beta$ for $r$ values corresponding to outbreeding ($r=0$) and inbreeding between outbred cousins ($r=1/8$), half-siblings ($r=1/4$), and full siblings ($r=1/2$). Across all parameter combinations, $m_{min}=1$ and $c=1$. Figure 1B shows how $m^{*}$ increases with both increasing $\beta$ and increasing $r$. Overall, optimal PI increases with increasing inbreeding depression and inbreeding, and the difference in magnitude of investment per offspring is often expected to be high for individuals that inbreed rather than outbreed (e.g., when $\beta=3.25$, $m^{*}_{r=1/2} > m^{*}_{r=0}$ by a factor of `r round(findm(low.guess=0,high.guess=5,rval=0.5,Beta=3.25) / findm(low.guess=0,high.guess=5,rval=0.0,Beta=3.25),digits=2)`). 
...
<!--- TODO: This needs to be fixed -- the figures are wrong, and we now need to get Fig. 4A in there somehow. Direct comparison is probably best, but I suspect I'll want to remove this later.--->
