...
In the above, $n$ is the total number of offspring a female produces in a reproductive bout. Biparental inbreeding models assume that this value is constant, but $n$ will scale linearly with $m$ (by definition) because females that invest more in each offspring (high $m$) will produce fewer total offspring (low $n$). To account for this, we can simply include $m$ to scale for offspring produced,
...
<!--- Note that we can expand this -- n * (\zeta_{off} / m) --->
