...
<!---  As $m \to \infty$, inbred offspring **always** have the same direct fitness (and therefore higher $\zeta_{off}$ due to IBD alleles) as their outbred counterparts; in other words, **I assume that sufficient parental investment can *always* compensate for being inbred** (though the amount of PI required might be extremely high). --->
...
The above figure shows how $\zeta_{off}$ increases with $m$ when given $r=0$ (solid curve) and $r=1/2$ (dashed curve). Given, $r=0$, $\zeta_{off}=0$ when $m=m_{min}$, meaning that offspring are only viable when $m>m_{min}$. Increasing $r$ effectively increases the minimum amount of parental investment required to produce a viable offspring to $m_{min}+\beta r$, so when $r=1/2$, $\zeta_{off}=0$ when $m=1.5$. Nevertheless, because inbred offspring carry more of their parents' alleles identical-by-descent, sufficiently high $m$ causes $\zeta_{off}$ of inbred offspring to exceed that of outbred offspring (represented by the intersection between solid and dashed curves in the figure above). The line running through the origin that is tangent to $\zeta_{off}(m)$ defines optimal investment for outbreeding $m^{*}_{r=0}=$ `r round(r00m,digits=2)` (solid line) and inbreeding $m^{*}_{r=1/2}=$ `r round(r05m,digits=2)` (dashed line) parents. The slope of each line is the rate of a parent's fitness increase for outbreeding given outbreeding $\gamma^{*}_{r=0}=$ `r round(r00g,digits=2)` (solid line) and inbreeding $m^{*}_{r=1/2}=$ `r round(r05g,digits=2)` (dashed line). To maximise fitness, females that inbreed with first order relatives should therefore invest more in offspring than females that outbreed ($m^{*}_{r=1/2}>m^{*}_{r=0}$). When both females invest optimally, inbreeding increases fitness more than outbreeding ($\gamma^{*}_{r=1/2}>\gamma^{*}_{r=0}$); this can also be confirmed by reducing our model to the results of established biparental inbreeding models (see Appendix 1).
...
Appendix 1: Consistency with biparental inbreeding models
----------------------------------------------

It is trivial to show that a female that inbreeds with a first order relative ($r=1/2$) has a higher fitness than a female that outbreed ($r=1/2$) given $m_{min}=1$, $\beta=1$, and $c=1$ at optimal values of $m^{*}_{r=1/2}$ and $m^{*}_{r=0}$. To do this, we define $\delta$ as follows,

$$ \delta_{r} = e^{-c(m^{*}_{r}-m_{min}-\beta r)}. $$

In biparental inbreeding models [e.g., @Kokko2006; @Parker2006; @Duthie2015a], it is assumed that $\delta_{r=0}=0$ for outbred offspring. Fitness from inbreeding to any degree $r$ can be determined by,

$$ \frac{1}{2}\left(1+r\right)\left(1-\delta_{r}\right). $$

As such, when $r=0$ and $\delta_{r=0}$, fitness is simply $1/2$ (or something proportional, depending on the model). This fitness value can be compared to that given inbreeding ($r>0$), with most models deriving the value of $\delta_{r}$ for which inbreeding returns a higher fitness then inbreeding avoidance. To show that inbreeding with first order relatives returns a higher fitness than outbreeding given the above conditions assumed in our model, we can simply use the above equation to compare the fitness given both $r=1/2$ and $r=0$, noting that $\delta_{r=1/2}=0.26$ and $\delta_{r=0}=0.32$ (note that $\delta_{r=1/2}>\delta_{r=0}$ because inbreeding parents are investing more in their offspring). Consequently, we can use the above equation to show that the fitness of an optimally investing female that inbreeds with a first order relative is $0.56$, compared with $0.34$ for the outbreeding female.
