...
While the fitness of a female that inbreeds with a first order relative ($r=1/2$) exceeds that of an outbreeding female ($\gamma^{*}=$ `r round(r00g,digits=3)`) when $\beta <$ `r round(findbeta(high.guess=10,rval=0.5,m=FALSE), digits=3)`, if the inbreeding female invests at the outbreeding female's optimum ($m^{*}_{r=0}$), then her fitness is higher only when $\beta <$ `r round(findbeta(high.guess=10,rval=0.5,m=r00m), digits=3)`. In contrast, if all females invest at $m^{*}_{r=1/2}$, then females that inbreeding to a lesser degree ($r<1/2$) or outbreed ($r=0$) have consistently lower $\gamma^{*}$ (Fig. 3C).
